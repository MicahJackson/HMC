...
######
# skipping it. Too much.
# Pima Indian Data
indian <- read.csv('https://archive.ics.uci.edu/ml/machine-learning-databases/pima-indians-diabetes/pima-indians-diabetes.data', header=FALSE)
indian <- indian[,c(3,6,8,9)]
names(indian) <- c('di.bp', 'bmi', 'age', 'disease')

scaled <- cbind(1,scale(indian[,(1:3)]))
X <- as.matrix(cbind(1, indian[,1:3]))
Y <- as.matrix(indian[,4])
mod <- glm(disease ~ di.bp + bmi + age, family='binomial', data=indian)
...
q.hat <- post.logistic.MAP(scaled,Y,rep(.01,4))
hess <- logistic.hessian(scaled,q.hat) + diag(c(S.inv%*%q.hat))

nx <- ncol(scaled)
A <- matrix(0,2*nx,2*nx)
A[1:nx,(nx+1):(2*nx)] <- diag(nx)
A[(nx+1):(2*nx),1:nx] <- -hess
...
std.pima <- logistic.HMC(scaled, Y, q.hat,S,M,20,.05, 2000)
system.time(logistic.HMC(scaled, Y, q.hat,S,M,20,.05, 2000))

split.pima <- split.HMC(scaled, Y, q.hat, hess, S, R,20, .05,2000)
system.time(split.HMC(scaled, Y, q.hat, hess, S, R,2, .2,2000))

splite.pima <- split.HMCe(scaled, Y, q.hat, hess, S, R,2, .2,2000, .9)
system.time(split.HMCe(scaled, Y, q.hat, hess, S, R,2, .2,2000, .9))
